...
namespace WSMGameStudio.Vehicles
{
    ...
    public class WSMVehicleController : MonoBehaviour
    {
        ...
        private void SpeedControl(float gearSpeedLimit, float speedUnitConversion)
        {
            ...
            _currentSpeed = _rigidbody.linearVelocity.magnitude * speedUnitConversion;
            if (_currentSpeed > gearSpeedLimit)
                _rigidbody.linearVelocity = Vector3.MoveTowards(_rigidbody.linearVelocity, maxSpeedFactor * _rigidbody.linearVelocity.normalized, 5f * Time.deltaTime);
        }
        ...
        {
            _rigidbody.AddForce(-transform.up * _downforce * _rigidbody.linearVelocity.magnitude);
        }
